...
def handle_pro(que):
    while True:
        ...
        if not home_tup:
            ...
            continue
        print('数据库中有 %s 的home_headers信息！获取headers信息并将其删除'% biz_msg)
        print('-------------------------------------------------------')
        time.sleep(1)
        #print(home_tup)
        ...
        conn.commit()
        print('%s 的home_headers信息已从数据库中删除，提取公众号ID信息'% biz_msg)
        print('-------------------------------------------------------')
        time.sleep(1)
        ...
        print('启动抓取程序')
        crawl_url(url_home,biz,url_msg2,main_id,sub_id,company_id,headers_home,headers_msg)


def crawl_url(url_home,biz,url_msg2,main_id,sub_id,company_id,headers_home,headers_msg):
    #更新数据库中数据状态
    print('连接数据库以更新任务状态...')
    while True:
        try:
