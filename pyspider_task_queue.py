...
class InQueueTask(DictMixin):
    __slots__ = ('taskid', 'priority', 'exetime')#限制绑定属性，该类实例只有这三个属性可以绑定
    ...
    #__cmp__是特殊函数，sorted()时被默认调用（排序默认函数）
    def __cmp__(self, other):
        if self.exetime == 0 and other.exetime == 0:
            return -cmp(self.priority, other.priority)#cmp(x,y) 函数用于比较2个对象，如果 x < y 返回 -1, 如果 x == y 返回 0, 如果 x > y 返回 1
        else:
...
class PriorityTaskQueue(Queue.Queue):
    ...
    def _put(self, item, heappush=heapq.heappush):
        ...
        if item.taskid in self.queue_dict:
            ...
            #将item导入self.queue这个heap和queue_dict中
            heappush(self.queue, item)
            self.queue_dict[item.taskid] = item

    def _get(self, heappop=heapq.heappop):
        while self.queue:
            #拿出并删除self.queue中最小的对象
...
class TaskQueue(object):
    ...
    def put(self, taskid, priority=0, exetime=0):
        ...
        #将一个task压入task queue
        now = time.time()
        #创建InQueueTask（即InQueue里的task对象）
        task = InQueueTask(taskid, priority, exetime)
        
        self.mutex.acquire()
        #如果self.priority_queue的queue_dict中有taskid这个key
        if taskid in self.priority_queue:
            ...
            #如果是新任务，根据exetime来将任务压入time_queue或者priority_queue
            if exetime and exetime > now:
                self.time_queue.put(task)
            else:
                self.priority_queue.put(task)
        self.mutex.release()
    #当有可利用线程时，从priority_queue队列中获取任务
    def get(self):
        ...
        self.mutex.release()
        return task.taskid
    #如果taskid in self.processing 将task标记为完成
